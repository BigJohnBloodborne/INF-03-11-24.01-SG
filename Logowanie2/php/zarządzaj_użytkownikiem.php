...
function rejestracja(){
    ...
    if($hashedPassword1 == $hashedPassword2){
        $count_user_query = "SELECT COUNT(user_email) AS count_email FROM projectt_user WHERE  user_email = '".$email."'";
        $result = mysqli_query($db, $count_user_query);
        $row = mysqli_fetch_array($result);
        if($row['count_email'] == 0){
        $query = "INSERT INTO projectt_user (user_name, user_surname, user_login, user_email, user_password)
            VALUES
            (?,?,?,?,?);";
...
function logowanie(){
    ...
    $stmt = mysqli_prepare($db, "SELECT * FROM projectt_user WHERE (user_login = ? OR user_email = ?) AND user_password = ?;");
